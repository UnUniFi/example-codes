...
export const postTxNftMarketMsgPlaceBid = async (chainID: string, restURL: string, websocketURL: string, mnt: string) => {
  ...
  const a = new ununifi.nftmarket.MsgCancelBid();
...
export const examplePostTxNftMarketMsgPlaceBid = async () => {
  ...
  const mnt =
    'chimney diesel tone pipe mouse detect vibrant video first jewel vacuum winter grant almost trim crystal similar giraffe dizzy hybrid trigger muffin awake leader';
